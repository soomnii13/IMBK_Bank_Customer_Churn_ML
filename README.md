...
### 3. 데이터 출처
- 캐글 [Bank Customer Churn Dataset](https://www.kaggle.com/datasets/gauravtopre/bank-customer-churn-dataset)  
  (row: 10000, col:12)
...
|------|-----------------------|-----------------------------------------------------------------------------|
| 1    | 불필요한 컬럼 제거     | 고객 고유 식별자(customer_id)는 예측에 불필요한 개별 식별 정보로 제거        |
| 2    | 결측치 처리           | 전체 데이터에서 결측치 여부를 확인 후 이상 없음 확인                        |
| 3    | 범주형 변수 인코딩     | country, gender 변수는 문자열 형태이므로 Label Encoding을 통해 수치형 변환   |
| 4    | 데이터 분할           | train / validation 데이터를 8:2 비율로 분할하고 stratify 적용으로 클래스 비율 유지 |
| 5    | 스케일링              | StandardScaler를 사용하여 데이터 정규화 수행                                |
...
- 클래스 불균형을 고려하여 Accuracy가 아닌 **F1-score**를 기준으로 모델을 선정  
